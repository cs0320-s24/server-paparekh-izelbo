...
public class CSVHandler implements Route {
  ...
  public Object handle(Request request, Response response) {
    ...
    String StateID = request.queryParams("participants");
    //     System.out.println(participants);


    int StateID = CountyDataUtilities.getStateID(State);

    // Creates a hashmap to store the results of the request
    Map<String, Object> responseMap = new HashMap<>();
    try {
      // Sends a request to the API and receives JSON back
      String county = this.sendRequest(Integer.parseInt(StateID));
      // Deserializes JSON into an Activity
      String[][] CountyData = CountyDataUtilities.deserializeCounty(county.json);
  ...
  private String sendRequest(int request)
    ...
    HttpRequest buildBoredApiRequest =
        HttpRequest.newBuilder()
            .uri(new URI("https://api.census.gov/data/2021/acs/acs1/subject" + request))
            .GET()
            .build();
